...
public class ReadRecordCypher extends Procedure {

  public final SQLStmt readStmtCypher =
  ...
  public void run(Connection conn, int keyname, String[] results) throws SQLException {
    ...
    String query = "MATCH (u:usertable) WHERE u.YCSB_KEY = " + keyname + " RETURN u";
    //System.out.println("Cypher read: " + query);
    ...
    try (Connection connection =
      ...
      if (connection.isWrapperFor(PolyConnection.class)) {
        ...
        RelationalResult result =
            polyStatement.execute("public", "cypher", query).unwrap(RelationalResult.class);

        // RelationalResult result = polyStatement.execute(query);

        String raw = result.toString(); // get raw document string
